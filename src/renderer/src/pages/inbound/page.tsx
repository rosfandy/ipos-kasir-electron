...
export function InboundPage() {
    ...
    const tableHead = ['no', 'products_barcode', 'products_name', 'quantity', 'units_name', 'received_date', 'supplier'];
    const aliases = { no: 'No', products_barcode: 'Barcode', products_name: 'Barang', quantity: 'Jumlah', units_name: 'Satuan', received_date: 'Tanggal', supplier: 'Supplier' };
    const LIMIT_PER_PAGE = 10;

    useEffect(() => {
        getTotalPages().then(() => fetchInbounds());
    ...
    const fetchInbounds = async () => {
        try {
            let response: any;
            if (selectedProduct) {
                response = await (window.api as any).inbound.getPaginated(page, LIMIT_PER_PAGE, selectedProduct.id, 'product_id', 'received_date', start, end);
                console.log('selected')
            } else {
                response = await (window.api as any).inbound.getPaginated(page, LIMIT_PER_PAGE, -1, null, 'received_date', start, end);
    ...
    const getTotalPages = async () => {
        try {
            ...
            const totalRecordsResponse = await (window.api as any).inbound.count(id, field, 'received_date', start, end);
